export function initActions(config) {
    ...
    window.lihatData = function(id) {
        if (detailUrl) {
            ...
            window.location.href = `/admin/master/dokumen-persyaratan/${id}`;
        }
    };
    ...
    window.editData = function(id) {
        if (editUrl) {
            ...
            window.location.href = `/admin/master/dokumen-persyaratan/${id}/edit`;
        }
    };
    ...
    window.hapusData = function (id, buttonEl) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const url = deleteUrl ? deleteUrl.replace(':id', id) : `/admin/master/dokumen-persyaratan/${id}`;
                    ...
                    .then(data => {
                        if (data.success) {
                            showToast(data.message || 'Data berhasil dihapus.', 'success')
                                .then(() => {
                                    // Logika reload table yang disesuaikan
                                    if (typeof window.dokumenPersyaratanTable !== 'undefined') {
                                        window.dokumenPersyaratanTable.ajax.reload(null, false);
                                    } else if (typeof window.dataWisudawanTable !== 'undefined') {
