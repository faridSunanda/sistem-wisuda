export function initActions(config) {
    ...
    window.lihatData = function(id) {
        if (detailUrl) {
            ...
            // Fallback URL
            window.location.href = `/admin/master/sesi/${id}`;
        }
    };
    ...
    window.editData = function(id) {
        if (editUrl) {
            ...
            // Fallback URL
            window.location.href = `/admin/master/sesi/${id}/edit`;
        }
    };
    ...
    window.hapusData = function (id, buttonEl) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const url = deleteUrl ? deleteUrl.replace(':id', id) : `/admin/master/sesi/${id}`;
                    ...
                    .then(data => {
                        if (data.success) {
                            showToast(data.message || 'Data berhasil dihapus.', 'success')
                                .then(() => {
                                    // Reload tabel Sesi
                                    if (typeof window.sesiTable !== 'undefined') {
                                        window.sesiTable.ajax.reload(null, false);
                                    } else {
                                        location.reload();
                                    }
                                });
