export function initActions(config) {
    ...
    window.lihatData = function(id) {
        if (detailUrl) {
            ...
            // Fallback URL ke admin.master
            window.location.href = `/admin/master/group/${id}`;
        }
    };
    ...
    window.editData = function(id) {
        if (editUrl) {
            ...
            // Fallback URL ke admin.master
            window.location.href = `/admin/master/group/${id}/edit`;
        }
    };
    ...
    window.hapusData = function (id, buttonEl) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const url = deleteUrl ? deleteUrl.replace(':id', id) : `/admin/master/group/${id}`;
                    ...
                    .then(data => {
                        if (data.success) {
                            showToast(data.message || 'Data berhasil dihapus.', 'success')
                                .then(() => {
                                    // Reload tabel Group jika ada
                                    if (typeof window.groupTable !== 'undefined') {
                                        window.groupTable.ajax.reload(null, false);
                                    } else {
                                        location.reload();
                                    }
                                });
