export function initActions(config) {
    ...
    window.lihatData = function(id) {
        if (detailUrl) {
            ...
            window.location.href = `/admin/wisuda/jadwal-pelaksanaan/${id}`;
        }
    };
    ...
    window.editData = function(id) {
        if (editUrl) {
            ...
            window.location.href = `/admin/wisuda/jadwal-pelaksanaan/${id}/edit`;
        }
    };
    ...
    window.hapusData = function (id, buttonEl) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const url = deleteUrl ? deleteUrl.replace(':id', id) : `/admin/wisuda/jadwal-pelaksanaan/${id}`;
                    ...
                    .then(data => {
                        if (data.success) {
                            showToast(data.message || 'Data berhasil dihapus.', 'success')
                                .then(() => {
                                    if (typeof window.jadwalPelaksanaanTable !== 'undefined') {
                                        window.jadwalPelaksanaanTable.draw();
                                    } else {
                                        location.reload();
                                    }
                                });
