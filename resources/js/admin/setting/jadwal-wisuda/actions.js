export function initActions(config) {
    ...
    window.lihatData = function(id) {
        const url = detailUrl ? detailUrl.replace(':id', id) : `/admin/setting/jadwal-wisuda/${id}`;
        window.location.href = url;
    };

    window.editData = function(id) {
        const url = editUrl ? editUrl.replace(':id', id) : `/admin/setting/jadwal-wisuda/${id}/edit`;
        window.location.href = url;
    };
    ...
    window.hapusData = function(event, id) {
        ...
        .then(data => {
            if (data.success) {
                showToast(data.message || 'Jadwal wisuda berhasil dihapus.', 'success');
