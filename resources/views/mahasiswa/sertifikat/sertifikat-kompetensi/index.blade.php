...
<div class="max-w-6xl mx-auto">
    ...
    <form action="{{ route('mahasiswa.sertifikat.kompetensi.store') }}" method="POST">
        @csrf
        <div class="bg-gray-40 rounded-lg shadow-sm border border-gray-200 p-4 md:p-6">
            ...
            <div id="sertifikat-container" class="space-y-6">
                ...
                @forelse($sertifikats as $sertifikat)
                    <div class="sertifikat-entry p-8 bg-white border border-gray-200 rounded-lg relative transition-shadow duration-200 hover:shadow-md">
                        ...
                        {{-- Tombol hapus (akan di-handle oleh JS) --}}
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input type="text" name="nama_sertifikat[]" id="nama_sertifikat_{{ $loop->index }}" value="{{ old('nama_sertifikat.' . $loop->index, $sertifikat->nama_sertifikat) }}" placeholder="Contoh: Web Programming Dasar" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 py-2 px-3" required>
                            </div>
                            <div>
                                <label for="penerbit_{{ $loop->index }}" class="block text-sm font-medium text-gray-700 mb-1">Penerbit</label>
                                <input type="text" name="penerbit[]" id="penerbit_{{ $loop->index }}" value="{{ old('penerbit.' . $loop->index, $sertifikat->penerbit) }}" placeholder="Contoh: Universitas Wahid Hasyim" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 py-2 px-3" required>
                            </div>
                            <div class="relative">
                                <label for="tanggal_terbit_{{ $loop->index }}" class="block text-sm font-medium text-gray-700 mb-1">Tanggal Terbit</label>
                                {{-- Format tanggal ke Y-m-d agar bisa dibaca input type="date" --}}
                    ...
                    <div class="sertifikat-entry p-8 bg-white border border-gray-200 rounded-lg relative transition-shadow duration-200 hover:shadow-md">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input type="text" name="nama_sertifikat[]" id="nama_sertifikat_1" value="{{ old('nama_sertifikat.0') }}" placeholder="Contoh: Web Programming Dasar" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 py-2 px-3" required>
                            </div>
                            <div>
                                <label for="penerbit_1" class="block text-sm font-medium text-gray-700 mb-1">Penerbit</label>
                                <input type="text" name="penerbit[]" id="penerbit_1" value="{{ old('penerbit.0') }}" placeholder="Contoh: Universitas Wahid Hasyim" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 py-2 px-3" required>
                            </div>
                            <div class="relative">
            ...
            <div class="flex items-center justify-between mt-6">
                <button type="button" id="add-sertifikat" class="inline-flex items-center gap-x-2 px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed">
                    <svg class="h-5 w-5" xmlns="http://www.w3.00/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
