...
<div class="max-w-6xl mx-auto">
    ...
    <form action="{{ route('mahasiswa.sertifikat.bahasa-internasional.store') }}" method="POST">
        @csrf
        <div class="bg-gray-40 rounded-lg shadow-sm border border-gray-200 p-4 md:p-6">
            <h2 class="text-xl md:text-2xl font-semibold text-gray-900 mb-6">Sertifikat Bahasa Internasional</h2>
                    ...
                    <div class="sertifikat-entry p-8 bg-white border border-gray-200 rounded-lg relative transition-shadow duration-200 hover:shadow-md">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input type="text" name="nama_sertifikat[]" id="nama_sertifikat_{{ $loop->index }}" value="{{ old('nama_sertifikat.' . $loop->index, $sertifikat->nama_sertifikat) }}" placeholder="Contoh: TOEFL ITP" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 py-2 px-3" required>
                            </div>
                            <div>
                                <label for="penerbit_{{ $loop->index }}" class="block text-sm font-medium text-gray-700 mb-1">Penerbit</label>
                                <input type="text" name="penerbit[]" id="penerbit_{{ $loop->index }}" value="{{ old('penerbit.' . $loop->index, $sertifikat->penerbit) }}" placeholder="Contoh: ETS" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 py-2 px-3" required>
                            </div>
                            <div class="relative">
                                <label for="tanggal_terbit_{{ $loop->index }}" class="block text-sm font-medium text-gray-700 mb-1">Tanggal Terbit</label>
                                <input type="date" name="tanggal_terbit[]" id="tanggal_terbit_{{ $loop->index }}" value="{{ old('tanggal_terbit.' . $loop->index, $sertifikat->tanggal_terbit->format('Y-m-d')) }}" placeholder="dd/mm/yyyy" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 py-2 px-3" required>
                    ...
                    <div class="sertifikat-entry p-8 bg-white border border-gray-200 rounded-lg relative transition-shadow duration-200 hover:shadow-md">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input type="text" name="nama_sertifikat[]" id="nama_sertifikat_1" value="{{ old('nama_sertifikat.0') }}" placeholder="Contoh: TOEFL ITP" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 py-2 px-3" required>
                            </div>
                            <div>
                                <label for="penerbit_1" class="block text-sm font-medium text-gray-700 mb-1">Penerbit</label>
                                <input type="text" name="penerbit[]" id="penerbit_1" value="{{ old('penerbit.0') }}" placeholder="Contoh: ETS" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 py-2 px-3" required>
                            </div>
                            <div class="relative">
                                <label for="tanggal_terbit_1" class="block text-sm font-medium text-gray-700 mb-1">Tanggal Terbit</label>
                                <input type="date" name="tanggal_terbit[]" id="tanggal_terbit_1" value="{{ old('tanggal_terbit.0') }}" placeholder="dd/mm/yyyy" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 py-2 px-3" required>
                                <div class="absolute inset-y-0 right-0 top-6 pr-3 flex items-center pointer-events-none">
                                    <svg class="h-5 w-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5" />
                                    </svg>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
            ...
            const newEntryHtml = `
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" name="nama_sertifikat[]" id="nama_sertifikat_${sertifikatCount}" placeholder="Contoh: TOEFL ITP" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 py-2 px-3" required>
                    </div>
                    <div>
                        <label for="penerbit_${sertifikatCount}" class="block text-sm font-medium text-gray-700 mb-1">Penerbit</label>
                        <input type="text" name="penerbit[]" id="penerbit_${sertifikatCount}" placeholder="Contoh: ETS" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 py-2 px-3" required>
                    </div>
                    <div class="relative">
        ...
        container.addEventListener('click', function (e) {
            const removeButton = e.targe.closest('.remove-sertifikat');
