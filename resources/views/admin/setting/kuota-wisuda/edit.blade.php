...
@section('content')
    <div class="space-y-4 md:space-y-6 pb-10">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl md:text-3xl font-bold text-gray-900">Edit Kuota Wisuda</h1>
        </div>

        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 md:p-8">
            <form action="{{ route('admin.setting.kuota-wisuda.update', $kuota->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                        ...
                        <select id="pendaftaran_wisuda_id" name="pendaftaran_wisuda_id" required
                            ...
                            @foreach ($jadwalPendaftarans as $jadwal)
                                <option value="{{ $jadwal->id }}"
                                    {{ old('pendaftaran_wisuda_id', $kuota->pendaftaran_wisuda_id) == $jadwal->id ? 'selected' : '' }}>
                                    {{ $jadwal->tahun_wisuda }} -
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="number" id="jumlah_kuota" name="jumlah_kuota"
                            value="{{ old('jumlah_kuota', $kuota->jumlah_kuota) }}" required min="1" max="1000"
                            class="w-full px-4 py-2.5 text-sm bg-gray-50 border-0 rounded-lg text-gray-900 font-medium focus:outline-none focus:ring-2 focus:ring-[#435ebe] focus:bg-white transition-all">
                        @error('jumlah_kuota')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
