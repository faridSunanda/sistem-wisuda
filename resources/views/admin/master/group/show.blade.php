...
@section('content')
    <div class="space-y-4 md:space-y-6 pb-10">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl md:text-3xl font-bold text-gray-900">Detail Data Group</h1>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 md:p-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                ...
                <div class="md:col-span-2">
                    <label class="block text-sm font-normal text-gray-600 mb-1.5">Nama Group</label>
                    <p class="text-base text-gray-900 font-bold">{{ $group->name }}</p>
                </div>

                <div class="md:col-span-2">
                    <label class="block text-sm font-normal text-gray-600 mb-1.5">Keterangan</label>
                    <p class="text-base text-gray-900 whitespace-pre-line">{{ $group->keterangan ?? '-' }}</p>
                </div>

                <div>
                    <label class="block text-sm font-normal text-gray-600 mb-1.5">Dibuat Pada</label>
                    <p class="text-base text-gray-900">{{ $group->created_at->format('d F Y H:i') }}</p>
                </div>
                <div>
                    <label class="block text-sm font-normal text-gray-600 mb-1.5">Diupdate Pada</label>
                    <p class="text-base text-gray-900">{{ $group->updated_at->format('d F Y H:i') }}</p>
                </div>
            </div>

            <div class="mt-6 pt-6 border-t border-gray-300 flex gap-3">
                <a href="{{ route('admin.master.group.edit', $group->id) }}"
