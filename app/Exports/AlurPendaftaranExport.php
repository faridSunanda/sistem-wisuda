...
class AlurPendaftaranExport implements FromCollection, WithHeadings, WithMapping, WithStyles, WithTitle, WithColumnWidths, WithEvents
{
    protected $data;
    ...
    public function map($row): array
    {
        ...
        $no_urut = is_object($row) ? ($row->no_urut ?? '') : ($row['no_urut'] ?? '');
        $judul = is_object($row) ? ($row->judul ?? '') : ($row['judul'] ?? '');
    ...
    protected function addTitleAndDate(Worksheet $sheet): void
    {
        ...
        $sheet->setCellValue('A1', 'Laporan Alur Pendaftaran');
    ...
    protected function applyAlignment(Worksheet $sheet): void
    {
        ...
        $sheet->getStyle('A4:B' . $highestRow)->applyFromArray($centerAlignment);
