...
class DokumenPersyaratanExport implements FromCollection, WithHeadings, WithMapping, WithStyles, WithTitle, WithColumnWidths, WithEvents
{
    protected $data;
    ...
    public function map($row): array
    {
        ...
        $tipe_dokumen = is_object($row) ? ($row->tipe_dokumen ?? '') : ($row['tipe_dokumen'] ?? '');
        $nama_dokumen = is_object($row) ? ($row->nama_dokumen ?? '') : ($row['nama_dokumen'] ?? '');
    ...
    protected function addTitleAndDate(Worksheet $sheet): void
    {
        ...
        $sheet->setCellValue('A1', 'Laporan Dokumen Persyaratan');
